...
Sura.map((sura, index) => {
    if (index === 0) return;
    const instance = new Suras(+sura[0], (+sura[1]), {
        english: sura[6],
        arabic: sura[4],
        finglish: sura[5],
    }, index, sura[7]);
    suraArray.push(instance);
})

const JsonFile = JSON.stringify(suraArray);

fs.writeFile(path.resolve(__dirname, "sura-data.json"), JsonFile, 'utf8', err => {
    if (err) {
